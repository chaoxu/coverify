...
def _is_not_found_error(err: Exception) -> bool:
    if getattr(err, "status", None) == 404:
        return True
    return "workspace not found" in str(err).casefold()


def _list_workspace_files_after_create(client: Any, workspace: str) -> Any:
    last_error: Exception | None = None
    for attempt in range(6):
        try:
            return client.list_workspace_files(workspace, branch="main")
...
def run_infinite_primes_workflow(
    *,
    ...
    tree = (
        _list_workspace_files_after_create(client, options.workspace)
        if options.create_workspace
        else client.list_workspace_files(options.workspace, branch="main")
    )
    ...
    if reviewer_client is not None:
        ...
        reviewer_client.review_pull_request(
            ...
            body="\n".join(
                [
                    "Coverify oracle reviewer submitted this verdict.",
                    "",
