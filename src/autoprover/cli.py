...
def build_client(
    *,
    ...
    base = CosheafClient(CosheafConfig(api_url=api_url))
    if token:
        return CosheafClient(CosheafConfig(api_url=api_url, token=token))
    if username and password:
        return CosheafClient(CosheafConfig(api_url=api_url, token=base.login(username, password)))
...
def cmd_login(args: argparse.Namespace) -> int:
    client = CosheafClient(CosheafConfig(api_url=args.api_url))
